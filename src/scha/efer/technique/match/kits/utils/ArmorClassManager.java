package scha.efer.technique.match.kits.utils;

import scha.efer.technique.TechniquePlugin;
import scha.efer.technique.match.Match;
import scha.efer.technique.match.kits.Archer;
import scha.efer.technique.match.kits.Bard;
import scha.efer.technique.match.kits.Rogue;
import scha.efer.technique.match.kits.utils.events.ArmorClassEquipEvent;
import scha.efer.technique.match.kits.utils.events.ArmorClassUnequipEvent;
import scha.efer.technique.profile.Profile;
import scha.efer.technique.util.events.ArmorEquipEvent;
...
public class ArmorClassManager implements Listener {
    ...
    public ArmorClassManager(TechniquePlugin plugin) {
            ...
            public void run() {
                for (Player player : Bukkit.getOnlinePlayers()) {
                    ...
                    if (match != null && match.isHCFMatch()) {
                        Bukkit.getScheduler().runTask(TechniquePlugin.get(), () -> {
                            attemptEquip(player);
                        });
                    }
                }
    ...
    public void attemptEquip(Player player) {
        ArmorClass current = TechniquePlugin.get().getArmorClassManager().getEquippedClass(player);
        if (current != null) {
            if (current.isApplicableFor(player)) {
                return;
            }

            TechniquePlugin.get().getArmorClassManager().setEquippedClass(player, null);
        ...
        Collection<ArmorClass> pvpClasses = TechniquePlugin.get().getArmorClassManager().getPvpClasses();
        for (ArmorClass pvpClass : pvpClasses) {
            if (pvpClass.isApplicableFor(player)) {
                TechniquePlugin.get().getArmorClassManager().setEquippedClass(player, pvpClass);
                break;
            }
        }
    }

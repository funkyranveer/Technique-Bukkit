...
import scha.efer.technique.match.kits.utils.bard.BardData;
import scha.efer.technique.match.kits.utils.bard.EffectData;
...
public class Bard extends ArmorClass implements Listener {
    ...
    private final TechniquePlugin plugin;

    public Bard(TechniquePlugin plugin) {
            ...
            bardData.heldTask = new BukkitRunnable() {
                ...
                public void run() {
                    ...
                    if (energy != 0 && energy != lastEnergy && (energy % 10 == 0 || lastEnergy - energy - 1 > 0 || energy == BardData.MAX_ENERGY)) {
                        lastEnergy = energy;
                        player.sendMessage(Color.translate("&5Bard Energy: &r" + energy));
                    }
                }
        ...
        if (match != null && (match.isHCFMatch() || match.isKoTHMatch())) {
            ...
            if (action == Action.RIGHT_CLICK_AIR || (!event.isCancelled() && action == Action.RIGHT_CLICK_BLOCK)) {
                ...
                if (bardData != null) {
                    ...
                    player.sendMessage(Color.translate("&5You have just used a &lBard Buff &5that cost you " + bardEffect.energyCost + " &5of your Energy."));
                }
            }
        }
    }
    ...
    private boolean canUseBardEffect(Player player, BardData bardData, EffectData bardEffect, boolean sendFeedback) {
        ...
        if (bardEffect.energyCost > currentEnergy) {
            errorFeedback = Color.translate("&5You do not have enough energy for this! You need " + bardEffect.energyCost + " energy, but you only have " + currentEnergy);
        }

        long remaining = bardData.getRemainingBuffDelay() / 1000;
        if (remaining > 0L) {
            errorFeedback = ChatColor.DARK_PURPLE + "You cannot use this for another " + ChatColor.BOLD.toString() + remaining + ChatColor.DARK_PURPLE + " seconds.";
        }

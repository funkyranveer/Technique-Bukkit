...
public class KoTHMatch extends Match {
    ...
    public void setupPlayer(Player player) {
        ...
        for (Player enemy : getOpponentTeam(team).getPlayers()) {
            NameTags.color(player, enemy, org.bukkit.ChatColor.DARK_PURPLE, false);
        }
    ...
    public void onStart() {
        ...
        }.runTaskTimerAsynchronously(TechniquePlugin.get(), 2L, 20L);
    }
    ...
    public boolean onEnd() {
        ...
        }.runTaskLater(TechniquePlugin.get(), 40L);
        ...
        loserInventories.append("Losers (Points: " + losingTeam.getKothPoints() + "): ").color(ChatColor.DARK_PURPLE);
        ...
        components.add(new ChatComponentBuilder("").parse("&5Post Match Inventories &f(Click to view)").create());
    ...
    public void onDeath(Player player, Player killer) {
        ...
        if (!teamPlayer.isDisconnected()) {
            ...
            player.sendMessage(CC.translate("&5You are now spectating for 8 seconds."));
            ...
            }.runTaskLater(TechniquePlugin.get(), 160L);
        } else {
            canEnd();
        }
    }

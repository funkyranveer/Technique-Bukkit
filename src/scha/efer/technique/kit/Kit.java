package scha.efer.technique.kit;
...
public class Kit {
    ...
    public static void init() {
        FileConfiguration config = TechniquePlugin.get().getKitsConfig().getConfiguration();
        ...
        for (String key : config.getConfigurationSection("kits").getKeys(false)) {
            ...
            if (config.contains(path + ".unranked-slot")) {
                kit.setUnrankedSlot(config.getInt(path + ".unranked-slot"));
            }

            if (config.contains(path + ".ranked-slot")) {
                kit.setRankedSlot(config.getInt(path + ".ranked-slot"));
            }
            ...
            kit.getGameRules().setClanranked(config.getBoolean(path + ".game-rules.clan-ranked"));
        ...
        kits.forEach(kit -> {
            if (kit.isEnabled()) {
                ...
                if (kit.getGameRules().isClanranked()) {
                    new Queue(kit, QueueType.CLAN);
                }
            }
        });
    ...
    public void save() {
        ...
        BasicConfigurationFile configFile = TechniquePlugin.get().getKitsConfig();
        configFile.getConfiguration().set(path + ".enabled", enabled);
        configFile.getConfiguration().set(path + ".unranked-slot", unrankedSlot);
        configFile.getConfiguration().set(path + ".ranked-slot", rankedSlot);
        ...
        configFile.getConfiguration().set(path + ".game-rules.clan-ranked", gameRules.isClanranked());

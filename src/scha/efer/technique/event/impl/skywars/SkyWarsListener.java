    ...
    public void onDeath(EntityDeathEvent event) {
        ...
        if (profile.isInSkyWars()) {
            if (event.getEntity() instanceof Player) {
                ...
                Bukkit.getScheduler().runTaskLaterAsynchronously(TechniquePlugin.get(), () -> event.getDrops().clear(), 10 * 20);
            }
        }
    }
                ...
                if (skyWars != null) {
                    if (skyWars.isWaiting() || skyWars.getState().equals(SkyWarsState.ROUND_ENDING)) {
                        if (event.getCause().equals(EntityDamageEvent.DamageCause.VOID)) {
                            event.getEntity().teleport(LocationUtil.deserialize(TechniquePlugin.get().getSkyWarsManager().getSkyWarsSpectators().get(0)));
                        }
                        event.setCancelled(true);
                    }
                }

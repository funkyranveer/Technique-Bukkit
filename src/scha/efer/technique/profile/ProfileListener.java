...
import scha.efer.technique.match.events.MatchEvent;
import scha.efer.technique.match.events.MatchStartEvent;
import scha.efer.technique.profile.meta.option.button.AllowSpectatorsOptionButton;
import scha.efer.technique.profile.meta.option.button.DuelRequestsOptionButton;
import scha.efer.technique.profile.meta.option.button.ShowScoreboardOptionButton;
...
public class ProfileListener implements Listener {
    ...
    public void onPlayerMatchStart(MatchEvent e) {
        if (e instanceof MatchStartEvent) {
            ...
            Bukkit.getScheduler().runTaskLaterAsynchronously(TechniquePlugin.get(), () -> match.getPlayers().forEach(player -> {
            ...
            if (e.getClickedBlock().getState() instanceof Sign) {
                ...
                if (sign.getLine(1) != null && sign.getLine(1).contains("[Click Here]")) {
                    if (sign.getLine(2).toLowerCase().contains("back to spawn")) {
                        TechniquePlugin.get().getEssentials().teleportToSpawn(e.getPlayer());
                    }
                }
            }
        ...
        if (event.getEntity() instanceof Player) {
            ...
            if (profile.isInLobby() || profile.isInQueue()) {
                ...
                if (event.getCause() == EntityDamageEvent.DamageCause.VOID) {
                    TechniquePlugin.get().getEssentials().teleportToSpawn((Player) event.getEntity());
                }
            }
        }
    }
    ...
    public void onPlayerJoinEvent(PlayerJoinEvent event) {
        ...
        TaskUtil.runAsync(() -> {
            ...
            TechniquePlugin.get().getEssentials().teleportToSpawn(p);
            profile.refreshHotbar();
            p.sendMessage(CC.translate(" "));
            p.sendMessage(CC.translate("&a&lSeason 3 &7(November 17th)"));
            p.sendMessage(CC.translate("&f → &eNew Ladders"));
            p.sendMessage(CC.translate("&f → &eNew Events"));
            p.sendMessage(CC.translate("&f → &eNew Perks"));
            p.sendMessage(CC.translate("&f → &eNew Arenas"));
            p.sendMessage(CC.translate(" "));
            p.sendMessage(CC.translate("&7&ohttp://store.strafe.world/ is currently on a 50% sale!"));
    ...
    public void onPlayerQuitEvent(PlayerQuitEvent event) {
        ...
        TaskUtil.runAsync(() -> {
            ...
            if (profile.getRematchData() != null) {
                Player target = TechniquePlugin.get().getServer().getPlayer(profile.getRematchData().getTarget());
